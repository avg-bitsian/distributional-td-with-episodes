...
def train_epoch(model, dataloader, loss_fn, alpha_plus, alpha_minus, optimizer=None, inactivation_indices=None, lmbda=0, reward_is_offset=True, auto_readout_lr=0.0):
    ...
    for batch, (X, y, x_lengths, trial_lengths, episode) in enumerate(dataloader):
        ...
        if model.predict_next_input:
            ...
            # y2 = torch.transpose(y, 1, 0)
            # print("Tensor y = ", y)
            # print("Y.shape = ", y.shape)
            # # print("Transposed y = ", y2)
            # print("Shape of transposed y = ", y2.shape)
        ...
        for i,l in enumerate(x_lengths):
            ...
            loss_for_episode = loss_fn(V_hat_2[:,i][:(l-1)], V_target_2[:,i][:(l-1)])
            # print("V_hat_2 = ", V_hat_2[:,i][:(l-1)])
            # print("V_target_2 = ", V_target_2[:,i][:(l-1)])
            # print("loss_for_episode ", i," = ", loss_for_episode)
            loss += loss_for_episode
...
def train_model(model, dataloader=None,
                experiment=None, batch_size=12, lr=0.003, alpha_plus=0.05, alpha_minus=0.05, lmbda=0,
                nchances=-1, epochs=5000, print_every=100,
    ...
    batch_losses = []
    scores[0], _ = train_epoch(model, dataloader, loss_fn, 
                               alpha_plus=alpha_plus, alpha_minus=alpha_minus, 
                               optimizer=None, lmbda=lmbda,
    ...
    try:
        for t in range(epochs):
            if t % print_every == 0:
                ...
                if test_dataloader is not None:
                    test_score, _ = train_epoch(model, test_dataloader, loss_fn, 
                                                alpha_plus=alpha_plus, alpha_minus=alpha_minus,
                                                optimizer=None,
                                                reward_is_offset=reward_is_offset,
                                                auto_readout_lr=auto_readout_lr)
            ...
            scores[t+1], batch_loss = train_epoch(model, dataloader, loss_fn, 
                                                  alpha_plus=alpha_plus, alpha_minus=alpha_minus,
                                                  optimizer=optimizer, 
        ...
        if use_weights == 'final':
            final_weights = model.checkpoint_weights()
            model.restore_weights(final_weights)
        ...
        print(f"Done! Final loss: {scores[-1]}")
        print(f"Done! Best loss: {best_score}")
        return scores, {'test_loss': test_scores, 'batch_losses': batch_losses}, weights

def probe_model(model, alpha_plus, alpha_minus, dataloader=None, experiment=None, inactivation_indices=None, reward_is_offset=True, auto_readout_lr=0.0):
    if experiment is not None:
        assert dataloader is None
