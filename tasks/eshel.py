...
from .trial import Trial, get_itis, RewardAmountDistribution, RewardTimingDistribution
...
class Eshel(Dataset):
    def __init__(self, 
                rew_size_distributions=[RewardAmountDistribution(DEFAULT_REW_SIZES), RewardAmountDistribution(DEFAULT_REW_SIZES)],
                rew_time_distributions=[RewardTimingDistribution(DEFAULT_REW_TIMES), RewardTimingDistribution(DEFAULT_REW_TIMES)],
        ...
        self.rew_time_distributions = rew_time_distributions
    ...
    def make_episodes(self, trials, ntrials_per_episode):
        ...
        for t in np.arange(0, len(trials)-ntrials_per_episode+1, ntrials_per_episode):
            ...
            for ti, trial in enumerate(episode):
                trial.trial_index_in_episode = ti
...
class Lowet(Eshel):
    def __init__(self, *args, **kwargs):
        none = RewardAmountDistribution([0], [1])
        fixed = RewardAmountDistribution([4], [1])
        vari = RewardAmountDistribution([2,6], [0.5,0.5])
        ...
        super().__init__(*args, **kwargs,
            ...
            rew_time_distributions=[RewardTimingDistribution(DEFAULT_REW_TIMES)]*len(rew_size_distributions),
